...
cat ./ats2019_account_rt_out.json | jq -rc '.result.records[] | select(.DeveloperName == ("ATS19Showroom","ATS19WholesalePartner")) | .DeveloperName + " " + .Id' | while read -r rtname rtid; do sed -i "s/$rtname/$rtid/g" ./ats19_account_showroom_small.json; done;
cat ./ats19_account_showroom_small.json
cat ./ats2019_account_rt_out.json | jq -rc '.result.records[] | select(.DeveloperName == ("ATS19Showroom","ATS19WholesalePartner")) | .DeveloperName + " " + .Id' | while read -r rtname rtid; do sed -i "s/$rtname/$rtid/g" ./ats19_account_wholesalepartner_small.json; done;
cat ./ats19_account_wholesalepartner_small.json
sfdx force:data:tree:import -p ./ats19_plan_small.json
rm ./ats19*.json